...
(defn- tmp [] (str (System/getProperty "java.io.tmpdir") "/aburi-ledger-test-" (gensym) ".edn"))
(defn- d1 [] [(k/add "surface:mobile-sdk" ":organism/kind" ":surface")
              (k/add "surface:mobile-sdk" ":surface/kind" ":embedded-sdk")])
(defn- d2 [] [(k/add "en.collector:adtech.exposes.surface:mobile-sdk" ":en/kind" ":exposes")
              (k/add "en.collector:adtech.exposes.surface:mobile-sdk" ":en/load" 0.7)])
  ...
  (let [p (tmp)]
    (try
      (let [c1 (k/append-tx (k/make-tx (d1) "t1" "as1" "") p)]
        ...
        (let [corrupted (str/replace (slurp p) ":exposes" ":relieves")]
          (spit p corrupted)
...
;; ── heartbeat (autorun) over the REAL committed tracker-exposure seed ────────

(deftest beat-ground-only-and-real-seed
  (let [ds (auto/ground-datoms)]
    (is (seq ds) "ground datoms are produced from the real committed tracker-exposure seed")
    (is (every? #(= ":db/add" (first %)) ds) "every datom is an :add (EAVT op)")
    (is (some (fn [[_ _ a]] (= ":en/from" a)) ds) "exposure 縁 are persisted (graph not just nodes)")
    (is (not-any? (fn [[_ _ a]] (str/starts-with? (str a) ":bond/")) ds)
        "GROUND only — derived :bond/* readouts not persisted (N1/G2)")))

(deftest beat-no-credential-or-pii
  ;; G8: exposure is STRUCTURE — no credential, no raw PII datoms
  (let [ds (auto/ground-datoms)]
    (is (not-any? (fn [[_ _ a]] (let [s (str a)]
                                  (or (str/includes? s "credential") (str/includes? s "password")
                                      (str/includes? s "token") (str/includes? s "email")
                                      (str/includes? s "raw-value") (str/includes? s "pii")))) ds)
        "no credential/raw-PII datoms in the ledger (G8, structure-only)")))
...
#?(:clj
   (when (= *file* (System/getProperty "babashka.file"))
     (let [{:keys [fail error]} (run-tests 'aburi.tests.test-kotoba)]
