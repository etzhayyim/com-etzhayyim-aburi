...
(ns aburi.methods.autorun
  ...
  permission/collector/datatype/relief NODES + their exposure :en/* 縁), and APPEND them as
  ONE content-addressed transaction to the append-only TRACKER-EXPOSURE LEDGER (kotoba.cljc).
  prev-cid chaining keeps the ledger tamper-evident + resume-safe.

  GROUND ONLY: tracking-exposure / relief readouts are DERIVED (computed on read, N1/G2) and
  are NEVER persisted. Deterministic by construction: the caller supplies tx-id + as-of (no wall
  clock); nodes emit in EDN-read order (node-order) + edges content-stable → resume-safe.
...
(defn- node-order [nodes]
  ;; de/load-file* attaches :aburi.methods.datom-emit/node-order; fall back to (keys nodes)
  (or (:aburi.methods.datom-emit/node-order (meta nodes)) (keys nodes)))
...
#?(:clj
   (defn default-seed-path []
     (str (io/file here-dir "data" "seed-tracker-exposure.kotoba.edn"))))
...
(defn beat
  "Run one heartbeat. opts:
     ...
     :log-path tracker-exposure-ledger path (required)
...
#?(:clj
   (defn -main [& args]
     (let [log-path (or (first args)
                        (str (io/file here-dir "data" "persisted" "aburi.tracker-exposure.cljc.kotoba.edn")))
           r (beat {:tx-id "aburi-beat-manual" :as-of "manual" :log-path log-path})]
       (println (str "tracker-exposure ledger head=" (:head r)
                     " datoms=" (:count r)
                     " appended=" (:appended r)
