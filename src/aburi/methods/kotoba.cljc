...
(ns aburi.methods.kotoba
  ...
  persistence layer the rest of the fleet now carries.

  Persists aburi's GROUND datoms — the surface / permission / collector / datatype / relief
  NODES and the exposure :en/* 縁 (PUBLIC structural facts, G8) — to a local append-only EDN
  log of content-addressed transactions. DERIVED state (tracking-exposure / relief readouts,
  the integral of incident 縁 computed on READ, N1/G2) is NEVER stored.
  ...
  appends to a local file. G8: exposure is STRUCTURE, never personal values — the ledger
  holds no credential and no raw PII; it records the tracking-exposure graph, routed to RELIEF."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; aburi 炙り — TRACKER-EXPOSURE LEDGER (append-only, content-addressed EAVT "
                        "commit-DAG of the surface/permission/collector/datatype/relief graph). Generated; "
                        "DO NOT hand-edit. Exposure is STRUCTURE — no credential, no raw PII (G8); routed to "
                        "RELIEF. ADR-2605312345.\n")))
